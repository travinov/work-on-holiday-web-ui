...
1. Новый пользователь указывает ФИО, обязательную рабочую почту и признак `Грейд 12+`. Почта должна быть корректной и не использоваться другим сотрудником.
2. По умолчанию требуется формат `Фамилия Имя Отчество`. Если отчества нет, пользователь устанавливает флаг `Нет отчества` и вводит `Фамилия Имя`.
3. Без флага обязательны ровно три кириллические части, с флагом — ровно две; буква `Ё` и дефис внутри части разрешены.
4. Система показывает персональный токен один раз.
5. При повторном входе пользователь вводит ФИО или рабочую почту и токен. Отдельное поле почты для регистрации повторно заполнять не требуется.
6. Если токен потерян, пользователь отправляет запрос и обращается к администратору за новым токеном.
7. Заблокированный или архивированный пользователь войти не может.

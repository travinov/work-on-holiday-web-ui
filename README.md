...
Реестр сотрудников и кабинет руководителя: [инструкция](docs/manager-workflow.md). Администратор импортирует CSV/XLSX с рабочей почтой и связями подчинения; руководитель просматривает задачи команды и сохраняет личные фильтры. Суперпользователь находится на вершине оргструктуры и имеет доступ ко всем сотрудникам и их заявкам через «Задачи команды — вся организация». Пустой руководитель в реестре означает прямое подчинение суперпользователю.

## Основной процесс

1. Администратор инициализирует БД.
2. Сотрудник входит по ФИО. При отсутствии отчества устанавливает соответствующий флаг. При первом входе получает персональный токен.
...
- `full_name_key` — стабильный ключ сотрудника (для старых записей — нормализованное ФИО, для новых записей реестра — отдельный идентификатор);
- `full_name` — ФИО;
...
## Ограничения полей Web-форм

Ограничения применяются одновременно в интерфейсе и на сервере:

| Поле | Ограничение |
|---|---:|
| ФИО и поиск по ФИО | 150 символов |
| Одна часть ФИО | 50 символов |
| Токен сотрудника при входе | 128 символов |
| Задача | 500 символов |
| Обоснование | 500 символов |
| Одна АС | 150 символов |
| Количество АС | 6 |
| Общая длина перечня АС | 1000 символов |
| Причина статуса сотрудника | 300 символов |
| Комментарий к закрытию периода | 500 символов |

При новой регистрации по умолчанию требуется `Фамилия Имя Отчество`. Флаг
`Нет отчества` разрешает зарегистрироваться с двумя частями: `Фамилия Имя`.
...
Для Production на том же сервере подготовлен отдельный профиль:

> Production-скрипты разрешено запускать только из отдельно проверенной и
> опубликованной Production-сборки. Текущую тестовую сборку ими устанавливать нельзя.

- текущий экземпляр остается на порту `8081`;
- Production запускается на порту `8082`;
- код, SQLite-БД, backup, env-файл, лог, `screen`-сессия и watchdog у экземпляров разные;
- первая установка Production создает новую пустую БД и не копирует данные текущего экземпляра.

Первая установка Production:

```bash
deploy/scripts/install-production-to-corporate-server.sh
```

Все последующие обновления Production:

```bash
deploy/scripts/update-production-corporate-server.sh
```

Подробная схема путей и команды проверки приведены в
...
Перед копированием новой версии и изменением схемы БД скрипт создаёт и проверяет комплект для ручного отката на сервере:

```text
$HOME/.local/state/work-on-holiday/backups/restore-points/pre-update-<дата-время-UTC>-<идентификатор>/
```

В комплект входят предыдущий код с Python-окружением `venv`, согласованная копия SQLite и SQL-dump, env-файл, скрипты запуска и watchdog, справочная копия crontab, инструкция и контрольные суммы. Комплект доступен только владельцу. Проверяются целостность базы, чтение архива, контрольные суммы и восстановление SQL-dump во временную базу. При любой ошибке обновление останавливается **до замены файлов**. Старые комплекты не удаляются автоматически.

Дополнительные команды подготовки не нужны: используйте скрипт обновления **из новой сборки**. Он сохраняет предыдущую версию непосредственно с сервера, даже если старый локальный ZIP уже заменён. Нужны свободное место для архива кода с `venv`, двух копий данных (SQLite и SQL) и временной проверки SQL-dump, а также доступ к штатным файлам установленного экземпляра. Если стандартные скрипты запуска не найдены, обновление остановится для уточнения конфигурации.

Откат выполняется вручную по [инструкции](deploy/MANUAL_ROLLBACK.md). Первичная установка не создаёт предыдущую версию: для обновления существующего экземпляра используется именно `update-corporate-server.sh`, а не `install-to-corporate-server.sh`.

После проверки комплекта скрипт:
...
В кабинетах администратора («Заявки») и руководителя («Задачи команды») доступна кнопка **«Выгрузить в Excel»** для текущего отфильтрованного списка. У руководителя сначала примените условия кнопкой «Показать задачи». Выгрузка включает статусы и фактическое время, сохраняет порядок списка и проверяет текущие права доступа. Подробнее — [выгрузка отфильтрованных заявок](docs/manager-workflow.md#выгрузка-отфильтрованных-заявок).

...
2. Для самостоятельной регистрации сотрудник указывает ФИО и рабочую почту. При отсутствии отчества устанавливает соответствующий флаг. Получает персональный токен и затем входит по ФИО или почте и токену. Для импортированных сотрудников первый токен выдаёт администратор.
...
## Единый интерфейс и работа в корпоративной сети
...
открыт кабинет. CDN, внешние шрифты и доступ к серверу разработки не нужны.
Для оформления не требуются Node.js, npm или сборка на корпоративном сервере.
Используются системные шрифты устройства. Версии URL ресурсов зависят от их
содержимого, чтобы после обновления браузер получил новое оформление.

Штатные install/update-скрипты копируют `static/` вместе с приложением;
скрипт обновления сохраняет прежние ресурсы в комплекте ручного отката.
Если перед приложением настроен отдельный reverse proxy, он должен пропускать
путь `/static/` в приложение. Подробности и проверки: [интерфейс](docs/local-interface.md).

## Структура БД

### `app_employee_directory`

Зарегистрированные сотрудники Web-only сценария. В актуальном процессе используются ФИО, рабочая почта и служебные идентификаторы. Телефоны, должность и числовой грейд не собираются и не проверяются.
...
- `full_name` — ФИО;
- `work_email` — обязательная рабочая почта при новой Web-регистрации; для старых записей без почты заполняется импортом при однозначном совпадении ФИО;
- legacy-поля `local_phone`, `mobile_phone`, `position_short_name`, `grade_num` сохраняются только для совместимости схемы и не используются Web-регистрацией;
...
| Рабочая почта при регистрации | 254 символа |
